...
{
public:
    node(const float x = 0.0f, const float y = 0.0f, const float z = 0.0f);
    virtual ~node();
    void attach_child(node* child);

    void update(const milliseconds delta_time);
...
private:
    ...
    void update(const milliseconds delta_time, glm::mat4& world_transform);
