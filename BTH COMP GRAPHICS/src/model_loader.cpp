...
void model_loader::readGeometries(std::vector<ColGeom>* v, const char *filename)
{
    tinyxml2::XMLElement *mesh, *vertices, *input, *source, *primitive;
            ...
            for(int i=0; i<7; i++)
            {
                ...
                if(primitive != NULL)
                {
                    ...
                    case 6:
                        ...
                        break;
                    default:
                        std::cout << "Primitive " << primitive_types[i] <<
...
void model_loader::freeGeometries(std::vector<ColGeom> *v)
{
